...
locals {
  ...
  recorded_subdomain = run_cmd(
    "--terragrunt-quiet",
    "bash", "-c",
    "out=$(aws ssm get-parameter --name /${local.project}/bootstrap/route53/subdomain --region ${include.root.locals.aws_region} --query Parameter.Value --output text 2>&1); rc=$?; if [ $rc -ne 0 ]; then case \"$out\" in *ParameterNotFound*|*\"Unable to locate credentials\"*|*NoCredentialProviders*) exit 0 ;; *) echo \"$out\" >&2; exit 1 ;; esac; fi; printf '%s' \"$out\""
  )
