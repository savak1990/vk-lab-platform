...
**Depends on:** every prior spec (001–012) — this is the integration point. Debezium (spec 025) lands after this spec; its lifecycle steps are added retroactively when that spec is implemented, not part of this spec's initial acceptance criteria.
...
- `make down`: `make argo-down` (cascading Argo-driven deletion in reverse order — public exposure → dependent workloads → stateful workloads → platform services/operators, gated on the `resources-finalizer.argocd.argoproj.io` finalizer and sync-wave-reversed pruning per spec 006-1) → verify Argo-managed platform is empty → Terragrunt destroy disposable → verify AWS-side disposable resources are gone → verify persistent resources remain untouched. (Once spec 025 (Debezium) lands, its deletion step slots in between "dependent workloads" and "stateful workloads," per the constitution's deletion-ordering rule — added at that point, not here.)
- Explicit postcondition verification for both directions, not just "the command exited 0."

Excludes: any new platform component — this spec orchestrates what specs 001–012 already built, it doesn't add new infrastructure (Debezium, spec 025, lands after this spec and is folded into the lifecycle sequence when it's implemented). Also excludes `make bootstrap-up`/`make bootstrap-down` (spec 001) and `make persistent-up`/`make persistent-down` (spec 002) themselves — this spec's `make up` calls the Persistent-existence check those specs define, but does not own creating or destroying Bootstrap or Persistent resources. Also excludes `make minikube-up`/`make kind-up` (spec 021, the `local` target) — those are separate, non-lifecycle-class commands outside the State/Bootstrap/Persistent/Disposable command surface this spec orchestrates.
...
7. `make up`/`make down` MUST be environment-agnostic — no hardcoded assumption that they run on a developer's machine (e.g. no reliance on local-only credential caches or interactive prompts) — so that spec 015's GitHub Actions workflows can call the exact same targets and get the exact same behavior (architecture.md §34). This spec builds only the local-invocable `make up`/`make down` targets; the GitHub Actions workflows that call them are out of scope here and covered in spec 015.
...
- This is the spec where all the individual destroy/recreate proofs from specs 005/007/024 should be re-run as one composed end-to-end flow, since the real acceptance test the constitution cares about (§38 in architecture.md) is the full sequence, not each component in isolation.
...
This spec's acceptance criterion **is** the constitution's full lifecycle acceptance test (architecture.md §38, constitution §11), run for real. Debezium lands after this spec (spec 025): until then, steps 2 and 5 below run without Debezium/CDC — that coverage is added retroactively when spec 025 is implemented, not part of this spec's own acceptance criteria.
...
5. (Deferred until spec 025 lands: verify CDC end-to-end — Debezium picks up the Postgres write and produces the Kafka event.)
6. `make down`
