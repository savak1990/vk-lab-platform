...
**Depends on:** every prior spec (001–012) — this is the integration point. Debezium (spec 024) lands after this spec; its lifecycle steps are added retroactively when that spec is implemented, not part of this spec's initial acceptance criteria.
...
This spec's acceptance criterion **is** the constitution's full lifecycle acceptance test (architecture.md §38, constitution §11), run for real. Debezium lands after this spec (spec 024): until then, steps 2 and 5 below run without Debezium/CDC — that coverage is added retroactively when spec 024 is implemented, not part of this spec's own acceptance criteria.
...
5. (Deferred until spec 024 lands: verify CDC end-to-end — Debezium picks up the Postgres write and produces the Kafka event.)
6. `make down`
