...
**Depends on:** 017-branch-protection (this workflow becomes the required status check reserved there)
...
Excludes: `terraform plan` against real state — that runs inside Atlantis (spec 018), which already comments plan output on the same PR; duplicating it here would produce two conflicting plan outputs and would require this workflow to hold AWS credentials it does not otherwise need. Also excludes the manually-triggered full lifecycle test (019) and the kind-based GitOps integration test (023) — that one spins up a real (if AWS-free) cluster and isn't "fast" in the same sense as the checks here, so it's a separate workflow, gated the same way as 019 (trusted-context PRs only), not folded into this one.
...
- A pull request cannot merge into `main` while this check is failing (confirmed via spec 017's required-status-check setting).
