...
**Depends on:** 002-persistent-foundation (Route 53 zone, ACM cert), 010-envoy-gateway (something for the ALB to route to)
...
This entire spec is `aws`-target-only. The `local` target (spec 020) has no ALB, Route 53, or ACM at all — access there is via `kubectl port-forward` directly to Envoy Gateway's Service (spec 010 Requirements 5–6), with no equivalent public-edge layer.

Excludes: any Gateway API routing logic (010 already owns this — this spec's ALB should route everything to Envoy Gateway, not duplicate host/path rules); the `lab.<root-domain>` hosted zone and certificate themselves (persistent, created in spec 002 — this spec only adds disposable records inside that zone); the parent/root hosted zone and its NS delegation (external, constitution §14 — never touched by this spec).
...
3. EKS MUST NOT be destroyed while ALB cleanup is still pending (constitution §7) — `make down`'s ordering (built out fully in spec 013) depends on this spec exposing a reliable signal for "ALB is gone" before the disposable Terraform stack proceeds.
...
- `make down` removes the ALB and its DNS records inside the lab zone, but the `lab.<root-domain>` hosted zone and its certificate remain — confirmed by re-querying the zone/certificate after teardown (formalized as a full acceptance scenario in spec 013).
