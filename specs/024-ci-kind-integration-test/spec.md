...
2. `make test`, `make kind-delete`, and `make kind-test` MUST be new Makefile targets built on spec 023's E2E suite; the GitHub Actions workflow MUST call these same targets rather than re-implementing any of their logic in workflow YAML — the workflow stays a thin wrapper (mirrors spec 016's "thin wrapper" requirement for `lab.yml`). A `make test-<service>` target MUST exist for each service that already has a Ginkgo label (spec 023) — do not add one ahead of the corresponding test file landing; a Makefile target pointing at a label that doesn't exist yet is dead weight.
