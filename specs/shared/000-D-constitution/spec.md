...
The platform supports a second execution target, `local` (minikube or kind), alongside the `aws` target described everywhere else in this constitution unless stated otherwise. See ADR 0006 and spec 022 for the full design.
...
- **§14 (DNS Domain Ownership and Configuration).** The delegated zone is `civo.<root-domain>`, a separate delegation from the AWS target's `lab.<root-domain>` (ADR 0027). This target has no ACM certificate. See the §8 variant above for how it handles TLS instead.
- **§16 (Resource Tagging).** Civo's tagging support is more limited than AWS's. The platform applies tags to Civo resources on a best-effort basis, where the Civo provider and its Terraform resource support it. These tags do not always carry every tag key this section requires for AWS resources.
- **§17 (Lifecycle Command Surface).** The same command pairs this section requires for AWS (`make state-up`/`down`, `bootstrap-up`/`down`, `persistent-up`/`down`, `up`/`down`, `full-up`/`full-down`) apply to the Civo target too. `PROVIDER` dispatches between them; there is no separate Civo command set (ADR 0027, spec CIVO-010). Civo introduces no new lifecycle command.
- **§19 (Fork Configurability).** A fork targeting Civo needs two more one-time setup steps before `PROVIDER=civo make up` can succeed: encrypt the Civo API token per ADR 0030, and complete the offline CA ceremony (ADR 0029, spec CIVO-080). These steps add to §19's existing AWS fork steps. They do not replace them.