...
| **§17 (Lifecycle Command Surface)** | The same command pairs this section requires for AWS (`make state-up`/`down`, `bootstrap-up`/`down`, `persistent-up`/`down`, `up`/`down`, `full-up`/`full-down`) apply to the Civo target too. `PROVIDER` dispatches between them; there is no separate Civo command set (ADR 0027, spec CIVO-010). Civo introduces no new lifecycle command. | The same command pairs apply, dispatched by `PROVIDER`, with no separate Hetzner command set and no new lifecycle command (ADR 0036, spec HETZ-010). Bootstrapping the control plane is part of `cluster-up`, not a command of its own — in fact every node bootstraps itself from cloud-init, and `cluster-up` only fetches the kubeconfig and waits for the nodes (ADR 0037). |
