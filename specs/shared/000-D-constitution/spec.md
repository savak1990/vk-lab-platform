...
All infrastructure MUST belong to exactly one lifecycle class. This applies to every real execution target — `aws`, `civo` and `hetzner` — with the per-provider classifications in §20. The `local` target's cluster and workloads are not governed by this taxonomy at all; see §18.
...
- shutdown MUST fail safely if persistence invariants are not satisfied.
  ...
  failure MUST warn loudly without blocking the shutdown (ADR 0032, ADR 0033). The
...
Exactly one GitHub OIDC provider MUST exist per AWS account, created once by the dedicated `make account-up` target and never recreated or destroyed by `make up`/`make down`/`make bootstrap-up`/`make bootstrap-down`. It is Bootstrap-lifecycle (§3) but account-global rather than per-project, so it MUST live in the separate `terraform/live/account/` layer: a second project applying the Bootstrap stack in the same account would otherwise attempt a duplicate provider and be rejected by AWS (see ADR 0021). It is account-level and region-agnostic — the same provider is reused regardless of which region a given stack deploys into. Individual IAM roles trusting that provider are a separate, per-consumer concern (e.g. a personal-lab deploy role, a privileged CI full-environment-test role) and MAY be created independently by whichever spec needs one; no spec MUST create a second provider (see ADR 0007).
...
Platform verification against a running environment (kind or real EKS) MUST be expressed as a real test suite (spec 023) reused across both targets, not duplicated bash scripts per environment.
...
The platform owns a delegated subdomain, `<SUBDOMAIN>.<root-domain>`, and an ACM certificate covering it and `*.<SUBDOMAIN>.<root-domain>`. Both are Persistent-lifecycle (§3).

`SUBDOMAIN` is a per-project variable defaulting to `lab`, not a fixed literal. One AWS account delegates from exactly one root domain, so `SUBDOMAIN` is what lets multiple projects coexist under it: each project owns its own zone and certificate. Two projects MUST NOT share a subdomain — the parent zone holds only one NS record set per name, so the second would contend for the first's delegation. `scripts/require-unique-subdomain.sh` MUST refuse that before any apply, and the delegation record MUST NOT set `allow_overwrite`, so even a bypassed apply fails rather than hijacking. The chosen value is recorded at `/<project>/bootstrap/route53/subdomain` and read back on later applies, so a forgotten re-export cannot silently move an existing zone (ADR 0023).
...
The platform owns a dedicated, Persistent-lifecycle VPC (`terraform/live/persistent/vpc`, spec 021), replacing the AWS account's default VPC used by earlier specs. Public subnets only, no NAT Gateway, no VPC interface endpoints — see ADR 0020 for the cost rationale.
...
- `Project = <PROJECT_NAME>` — identifies the resource as belonging to this repository/platform, distinct from any other project or unrelated resource in the same AWS account. The value is the project name the stack was applied under (default `vk-lab-platform`), not a fixed literal: a project name is free-form, and concurrent projects in one account are distinguished by this tag (ADR 0022 as amended).
...
- `make park` takes a Disposable cluster's worker nodes to zero and `make unpark` brings them back, leaving the control plane, etcd, every Argo CD object, the volumes and the load balancer with its DNS records in place. Like `full-up`/`full-down` they are a guarded modifier on an existing pair and not a new lifecycle class: they create and destroy nothing outside the Disposable class `make up`/`make down` already own, and they change no other command's contract or guards. A parked cluster is still Disposable. Both MUST refuse on a target whose arm is not implemented, naming the reason rather than only refusing; MUST require the cluster to exist and its API to be reachable, because a park that cannot drain MUST NOT destroy a worker; and MUST NOT delete Argo CD's root Application — `make cluster-down` refuses while root exists, and a parked cluster's surviving servers and volumes still carry the labels a teardown sweep matches on, so removing root would arm the next `make down` to delete a live cluster. A parked cluster still costs money, so park is for hours and `make down` is for weeks. See `specs/hetzner/200-D-parked-clusters/` and ADR 0041.
- `make clusters` is read-only and project-independent: it lists every platform cluster live in each provider's account — EKS clusters in the AWS account, Kubernetes clusters in every Civo region the catalogue lists, and `scope=platform` servers grouped by their `project` label in the Hetzner account — across all projects and all providers in one invocation, and MUST NOT read any project's Terraform state, require a `PROJECT_NAME`, or modify the operator's kubeconfig. It MUST NOT be narrowed by an inherited `PROVIDER`, `PROJECT_NAME` or `REGION`; the Makefile exports all three into every recipe, so it sweeps every provider whatever it was invoked with. A provider whose CLI or credentials are unavailable MUST be reported as skipped, never omitted silently — silence from this command reads as "nothing is running", which is the one answer that costs money. It creates and destroys nothing, so it is outside the one-command-per-class rule. See `specs/shared/046-D-multi-provider-cluster-inventory/`.
- `make kubeconfig` and `make test-kubeconfig` are the only commands that MAY write the operator's default kubeconfig or change its current context. Both exist for that purpose and are invoked deliberately. Every other command MUST work through a repo-local kubeconfig selected with `KUBECONFIG` (`scripts/lib/provider.sh`'s `use_isolated_kubeconfig`), so a bring-up, a teardown or a test run never moves the operator's kubectl off the cluster they are working on. A library function that runs an unqualified `kubectl` MUST call `require_isolated_kubeconfig` first, so a missing isolation call fails loudly instead of reaching the operator's selected cluster. See ADR 0034.
- The `local` target (§18) is selected by `PROVIDER=local` and uses the same command pairs as every other target, exactly as §20 requires of Civo and Hetzner. It adds no lifecycle command of its own. `make bootstrap-up`/`-down` and `make persistent-up`/`-down` are no-ops there, because that target owns no cloud resources; `make cluster-up` creates its kind cluster and `make cluster-down` deletes it. See ADR 0038.
...
The platform supports a development-only execution target, `local` (kind), alongside the `aws` target described everywhere else in this constitution unless stated otherwise. The real non-EKS targets are covered by §20; `local` is not one of them, and §20's per-provider variants do not apply to it. It is selected by `PROVIDER=local` and shares §17's command surface. See ADR 0038 and spec 022 for the full design.

The `local` target MUST make no AWS API call at all. It MUST NOT read Parameter Store, assume a role, or decrypt a committed ciphertext through KMS. The Kubernetes `Secret` objects the platform needs are created at bring-up with generated, throwaway values, which is sufficient because everything on this target is discarded with the cluster.
...
- **§5 (Security)** — the `local` target MUST NOT use SSM Parameter Store, EKS Pod Identity, IAM Roles Anywhere, or External Secrets. Its secrets mechanism — generated placeholder values written directly into Kubernetes `Secret` objects at bring-up — is defined in spec 022, not this section. No credential it holds is real, so none is worth protecting.
...
**Every AWS resource this platform creates lives in `eu-west-1`, whatever the target.** A `civo` or `hetzner` project's AWS-side resources — its state bucket, its SSM parameters, its Roles Anywhere chain — stay there regardless of where its cluster runs. There is no project AWS region distinct from the account one, and no code MUST introduce a second AWS region value.

**`REGION` selects the Civo region or the Hetzner location, and nothing else.** On `aws` it is not an input: a value other than `eu-west-1`, in any case, MUST be refused offline with a message stating the rule, never silently ignored and never silently honored (ADR 0040). `local` owns no cloud resources and ignores it. `REGION` MUST be validated before any cloud call and without credentials, against `scripts/lib/catalog.sh`, which is keyed by (provider, region) because node-type availability varies by region. It is matched case-insensitively and canonicalised to the provider's own spelling. It reaches Terraform through one `get_env` per unit and MUST NOT be recomputed anywhere else, and it MUST NOT be taken from ambient `AWS_REGION`/`AWS_DEFAULT_REGION` or an operator's AWS profile.

A project's state bucket and backup bucket carry the *provider's* region in the name, so that a region change cannot silently build a second platform against a bucket that already exists elsewhere. On `aws` that component is always `eu-west-1`.

---

## 20. Non-EKS execution targets

The `PROVIDER` operator input selects a real (non-`local`) execution target other than AWS/EKS: Civo managed Kubernetes (ADR 0027) or Hetzner Cloud (ADR 0036, whose bootstrap mechanism is replaced by ADR 0037). Neither is a substitute for §18's `local` target. `local` remains AWS-free by design. Both are real cloud providers, each with its own AWS-adjacent identity and DNS surface. §3, §4, §7, and §17 apply to both targets exactly as written, with no exemption. The sections in the table below carry an explicit per-provider variant instead of a blanket carve-out; a section absent from the table applies as written on every target.

| Section | Civo variant | Hetzner variant |
|---|---|---|
| **§3 (Lifecycle Separation)** | The Civo cluster, its node pool, Argo CD, and every Kubernetes workload on it are Disposable, exactly as the equivalent AWS resources are. The Civo network and reserved IP are Persistent. IAM Roles Anywhere's trust anchor, profile, and roles (ADR 0029) are Bootstrap. See ADR 0027. | The Hetzner servers — including the platform-owned k3s control plane — the firewall, Argo CD, and every Kubernetes workload are Disposable. Owning the control plane is not a reason to keep it. The Hetzner network, its subnet, and the SSH key are Persistent. Roles Anywhere's trust anchor, profile, and roles are Bootstrap. There is no reserved IP: the load balancer owns its own address and takes a new one on each `make up`. See ADR 0036 and, for the bootstrap mechanism, ADR 0037. |
| **§5 (Security)** | Civo workloads reach AWS through IAM Roles Anywhere (ADR 0029), not EKS Pod Identity — the mechanism differs. The intent does not differ: neither target keeps a static AWS credential at rest in the cluster. The Civo API token (`CIVO_TOKEN`) is not an AWS credential. This section's GitHub-Actions-OIDC clause does not directly apply to it. But `CIVO_TOKEN` is itself a long-lived static key. ADR 0030 handles it with the same KMS-encrypted-ciphertext discipline this section already requires for other secrets. | Identical in mechanism and intent: Roles Anywhere, and no static AWS credential at rest in the cluster. The Hetzner API token is not an AWS credential either, and ADR 0030 covers it under the same ciphertext discipline. One difference is material. The cloud controller manager and the CSI driver read a provider token from inside the cluster, so on this target a provider token MUST exist there. It MUST be a second token dedicated to in-cluster use, never the operator's, held in `kube-system/cloud-operator-secret` and created untracked by `argo-up` (ADR 0030 as amended, ADR 0036). |
| **§8 (Public Traffic)** | Civo has no NLB or ACM equivalent. TLS terminates at Envoy Gateway using cert-manager and Let's Encrypt HTTP-01 (ADR 0028), not at the load balancer. The Civo load balancer, like the AWS NLB, performs no host/path routing. That discipline is unchanged. | Hetzner has no NLB or ACM equivalent either. An hcloud load balancer carries plain TCP to Envoy Gateway, which terminates TLS with cert-manager. It performs no host/path routing, exactly as the AWS NLB and the Civo load balancer do not. A Hetzner firewall attaches to servers only and cannot protect a load balancer, so the load balancer is reachable on its configured listener ports and any restriction happens inside the cluster. |
| **§9 (Compute and Cost)** | Civo has no Karpenter. Dynamic capacity comes from the upstream Kubernetes cluster autoscaler on the single `workers` pool, and MUST stay bounded the same way: 3 minimum nodes and 4 maximum `g4s.kube.medium` nodes (spec CIVO-170), now stated as `MIN_WORKER_NODES` and `MAX_WORKER_NODES` (spec SHARED-047). Civo's autoscaler owns the pool Terraform created, so both bounds are absolute rather than a delta. Terraform creates the pool at the minimum and then stops owning the count. The floor is 3 because the platform's measured working set does not fit on 2 nodes, and a lower floor lets the autoscaler — which simulates on requests, not use — take the cluster into permanent over-subscription. Widening either bound needs explicit justification recorded in that spec or an ADR, exactly as §9 requires for Karpenter. | Hetzner has no Karpenter either, and the platform owns the control plane. Two bounds apply. The control plane is exactly one server, tainted so it carries no workload, sized separately from the workers (spec HETZ-178). Worker capacity is the upstream cluster autoscaler on a node group separate from the fixed pool, bounded at 0 minimum and 1 maximum (spec HETZ-170), on top of a fixed pool of 1. Both come from `MIN_WORKER_NODES` and `MAX_WORKER_NODES` (spec SHARED-047), which count worker nodes only; because that group sits beside the fixed pool rather than containing it, the autoscaler's ceiling is `MAX_WORKER_NODES - MIN_WORKER_NODES` and its floor is always 0. `MAX_WORKER_NODES` above 3 is refused offline for the same server-cap reason. The binding limit is not cost but Hetzner's 5-servers-per-account cap, shared with CI: 1 control plane + 1 fixed worker + 1 autoscaled leaves 2 for a CI run. Widening either bound needs explicit justification recorded in that spec or an ADR, exactly as §9 requires for Karpenter. |
| **§14 (DNS Domain Ownership and Configuration)** | The delegated zone is `civo.<root-domain>`, a separate delegation from the AWS target's `lab.<root-domain>` (ADR 0027). This target has no ACM certificate. See the §8 variant above for how it handles TLS instead. | The delegated zone is `hz.<root-domain>`, a third separate delegation (ADR 0036). This target has no ACM certificate. See the §8 variant for how it handles TLS instead. |
| **§16 (Resource Tagging)** | Civo's tagging support is more limited than AWS's. The platform applies tags to Civo resources on a best-effort basis, where the Civo provider and its Terraform resource support it. These tags do not always carry every tag key this section requires for AWS resources. | Hetzner accepts a key/value label map, so this section is satisfied in full. Every `hcloud_*` resource carries `project`, `scope`, `lifecycle` and `managed_by`. The lowercase snake-case spelling is a house convention, not a provider constraint — Hetzner permits uppercase keys, so the capitalised AWS spelling would be accepted and MUST still not be used here. Resources the controllers create carry the labels those controllers choose, which are hyphen-cased; a leak sweep MUST match both spellings. |
| **§17 (Lifecycle Command Surface)** | The same command pairs this section requires for AWS (`make state-up`/`down`, `bootstrap-up`/`down`, `persistent-up`/`down`, `up`/`down`, `full-up`/`full-down`) apply to the Civo target too. `PROVIDER` dispatches between them; there is no separate Civo command set (ADR 0027, spec CIVO-010). Civo introduces no new lifecycle command, and its `make park`/`unpark` arm is not implemented (spec HETZ-200 §11). | The same command pairs apply, dispatched by `PROVIDER`, with no separate Hetzner command set (ADR 0036, spec HETZ-010). This target is the one exception to "no new lifecycle command": it adds `make park`/`make unpark`, which §17 permits as a guarded modifier on the Disposable pair rather than a new lifecycle class (spec HETZ-200, ADR 0041). Bootstrapping the control plane is part of `cluster-up`, not a command of its own — in fact every node bootstraps itself from cloud-init, and `cluster-up` only fetches the kubeconfig and waits for the nodes (ADR 0037). |
| **§19 (Fork Configurability)** | A fork targeting Civo needs two more one-time setup steps before `PROVIDER=civo make up` can succeed: encrypt the Civo API token per ADR 0030, and complete the offline CA ceremony (ADR 0029, spec CIVO-080). These steps add to §19's existing AWS fork steps. They do not replace them. | A fork targeting Hetzner needs four more one-time setup steps before `PROVIDER=hetzner make up` can succeed: create a dedicated Hetzner project; encrypt its two API tokens per ADR 0030, the operator's and the dedicated in-cluster one, each in its own ciphertext file as this constitution's §5 requires; encrypt an SSH private key for node access; and complete the offline CA ceremony (ADR 0029, spec HETZ-080). These add to §19's existing AWS fork steps. They do not replace them. |
