...
| **§9 (Compute and Cost)** | Civo has no Karpenter. Dynamic capacity comes from the upstream Kubernetes cluster autoscaler on the single `workers` pool, and MUST stay bounded the same way: 2 minimum nodes and 3 maximum `g4s.kube.medium` nodes (spec CIVO-170). Terraform creates the pool at the minimum and then stops owning the count. Widening either bound needs explicit justification recorded in that spec or an ADR, exactly as §9 requires for Karpenter. | No variant recorded here. §9 applies as written until spec HETZ-170 sets an autoscaler bound for this target. |
