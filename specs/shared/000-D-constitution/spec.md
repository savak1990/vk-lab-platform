...
- `make clusters` is read-only and project-independent: it lists every platform EKS cluster live in the AWS account, across all projects, and MUST NOT read any project's Terraform state, require a `PROJECT_NAME`, or modify the operator's kubeconfig. It creates and destroys nothing, so it is outside the one-command-per-class rule.
