...
- One IAM role per Terragrunt stack (`bootstrap`, `persistent`, `disposable`, `ci/persistent`) that Atlantis assumes per-project, mirroring spec 001's per-stack role split. `ci/disposable` is deliberately excluded here: spec 018's own workflow creates and destroys it on every full-lifecycle run, via its own OIDC-authenticated role. That's a test lifecycle, not a routine Terraform edit — Atlantis's plan-on-PR/apply-on-merge model doesn't fit it, so Atlantis holds no role for it.
...
Excludes: the fast, lint-only PR checks that don't need real AWS credentials (017 — those run independently of Atlantis, on every PR, even ones Atlantis has nothing to plan); the kind-based GitOps integration test (022 — no AWS involved at all); the manually-triggered full create→destroy→recreate→destroy lifecycle test (018 — Atlantis only ever runs ordinary `plan`/`apply` against the platform's own stable stacks, it does not orchestrate or hold any role for that test's `ci/disposable` environment); any Kubernetes/GitOps resource (Argo CD's job, constitution §2 — Atlantis touches Terraform/Terragrunt state only, never `gitops/`).

Atlantis's routine plan/apply role and spec 018's privileged full-environment-test role are deliberately separate mechanisms serving different purposes: Atlantis reviews and applies ordinary Terraform edits to stable stacks via its own instance/task role (no OIDC); spec 018 stands up and tears down an entire ephemeral AWS environment per PR via its own OIDC-authenticated role (architecture.md §17a). Neither substitutes for the other.
...
2. Atlantis MUST use a dedicated IAM role per Terragrunt stack (`bootstrap`, `persistent`, `disposable`, `ci/persistent`), least-privilege per stack — MUST NOT use one broad role with access across all state paths. Atlantis MUST NOT hold a role for `ci/disposable`. Spec 018's own workflow applies and destroys that state on every full-lifecycle run, using spec 018's own OIDC role (§17a of architecture.md) — never through Atlantis's plan/apply-on-merge flow.
...
4. Atlantis MUST run `plan` automatically when a pull request touching `terraform/**` is opened or updated, and MUST run `apply` only after this repository's required PR approval (spec 015) — MUST NOT apply automatically on every push without review.
...
- Merging that PR (after spec 015's required approval) triggers `atlantis apply` (or an explicit `atlantis apply` PR comment, per the chosen workflow mode), applying only the intended stack.
