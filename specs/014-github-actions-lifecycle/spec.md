...
**Depends on:** 001-bootstrap (state backend/tagging conventions this spec's own Terraform follows, and — since ADR 0007 — the GitHub OIDC provider this spec's role trusts), 013-lifecycle (the `make up`/`make down` targets this spec calls)
...
Gives the platform's lifecycle commands a GitHub Actions entry point, so `make up`/`make down` can be triggered from GitHub and not only from a developer's machine, per architecture.md §34's "Workstation-Initiated and GitHub Lifecycle Equivalence" (this is about the `aws` target run from GitHub vs. a workstation — unrelated to the `local`/minikube-kind target from spec 020):

- A personal-lab-scoped IAM role trusting the GitHub OIDC provider spec 001 creates — the first real consumer of GitHub OIDC federation in this platform (spec 016's Atlantis uses its own instance/task role instead, never OIDC).
...
3. These workflows operate on the **personal** lab's persistent/disposable state, not CI's isolated `ci/persistent`/`ci/disposable` state (constitution §11) — the OIDC role they assume MUST be scoped to the personal-lab Terraform state paths, distinct from spec 018's CI-scoped role and from spec 016's per-stack Atlantis roles.
...
5. `make down` runs (whether local or via `lab-down.yml`) MUST still respect every postcondition and safety check spec 013 already defines — this spec adds a trigger, not a second implementation of shutdown logic.
...
- Authenticate with `aws-actions/configure-aws-credentials` using this spec's own OIDC role — do not reuse the CI-scoped role from spec 018 or Atlantis's per-stack roles from spec 016.
- Keep the workflow YAML itself minimal: checkout → configure AWS credentials → `make up` (or `make down`) → surface the command's own output/logs as the workflow's result, rather than re-implementing any status reporting spec 013 already does.
...
- Manually triggering `lab-up.yml` from the GitHub Actions UI produces the same healthy end state as running `make up` locally (verified against spec 013's own health checks).
- Manually triggering `lab-down.yml` produces the same clean teardown as running `make down` locally, including all of spec 013's postcondition checks passing.
