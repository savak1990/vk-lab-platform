...
**Status note:** Implemented — Req 3's destroy/recreate proof exists as a runnable playbook (`tests/manifests/005-storage-contract/`), not an archived executed run
...
This spec, and its `Retain`-reclaim-policy/EBS-rebind requirements below, apply to the `aws` target only. The `local` target (spec 022) uses the cluster's default local StorageClass (hostpath/local-path) with `Delete` reclaim semantics instead — the deliberate inverse of Requirement 2 below — since local data is fully throwaway and needs no rebind procedure.

Excludes: the actual Postgres (007) and Kafka (025) deployments — this spec only proves the underlying mechanism they'll rely on.
...
4. The process for recreating a cluster and rebinding/restoring a retained EBS volume to a new PVC MUST be documented as part of this spec's output, since spec 007 and 025 will follow this exact procedure for real data.
...
- Document the rebind procedure (manual PV recreation with the retained volume's ID, or a controller-assisted approach) clearly enough that spec 007/025 can follow it verbatim for real Postgres/Kafka volumes.
