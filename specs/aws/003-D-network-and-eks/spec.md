...
**Status note:** Implemented — acceptance criteria superseded by ADR 0020 (platform-owned VPC replaces the AWS default VPC)
...
Creates the EKS cluster and its fixed system capacity, **in the AWS account's default VPC** — a dedicated persistent VPC is explicitly out of scope until spec 021:
...
Excludes: any dedicated/custom VPC (spec 021), Karpenter itself (006 — Karpenter is Argo-managed, this spec only provides the fixed system capacity it runs on), Argo CD installation (004), any workload.
