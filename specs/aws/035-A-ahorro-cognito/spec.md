...
8. `lab-role` MUST gain `cognito-idp` permission: it had none at all, so the lifecycle button could neither apply nor destroy this unit. `CreateUserPool` MUST sit in its own statement on `Resource: "*"` — no pool ARN exists before the pool does, the same AWS IAM limitation the file already documents for `s3:CreateBucket` and `ec2:Create*`. Every other action addresses an existing pool and MUST stay scoped to `arn:aws:cognito-idp:*:<account>:userpool/*`. A local apply runs as the operator, so only a lifecycle run exercises this.
