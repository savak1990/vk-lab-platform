...
Not applicable to the `local` target (spec 022): kind uses fixed local nodes with no dynamic provisioning, and Karpenter is omitted from the `local` target's rendered app list entirely.
