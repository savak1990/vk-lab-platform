...
6. The design MUST hold in the future `local` target (spec 022, kind) with no cloud-specific logic — `kubectl delete application root --cascade=foreground` is the same command regardless of where the cluster runs.
...
- Still unverified as of 2026-09-06. A teardown failed that day in a way that looks like this assumption breaking but is not: a commit landed on the tracked branch mid-teardown, Argo started a sync, and a completed hook `Job` kept its `argocd.argoproj.io/hook-finalizer` after Argo stamped its `deletionTimestamp`. That unreapable Job held the sync operation `Running`, and Argo processes no deletion finalizer — including `root`'s — while an operation is in flight. The prune never started at all (the four lowest-wave Applications never received a `deletionTimestamp`), so the question above went untested. Do not close it, and do not add per-wave wait loops on the strength of that incident.
- Deleting `root` has a precondition the cascade cannot recover from on its own: no sync operation may be in flight, and no hook resource may be holding an orphaned `argocd.argoproj.io/hook-finalizer`. `argo-down.sh` now disarms automated sync as its first action against a reachable cluster, aborts any operation already `Running` by setting `status.operationState.phase: Terminating`, and releases orphaned hook finalizers immediately before the cascade delete.
