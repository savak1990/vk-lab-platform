...
4. Once spec 018's fast-validation workflow exists, its status check MUST be added as a required check on `main` — this spec does not block on 018 existing yet, but the required-check setting MUST be revisited and updated when 018 lands, not left permanently empty.
