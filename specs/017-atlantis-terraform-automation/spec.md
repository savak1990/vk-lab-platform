...
2. Atlantis MUST use a dedicated IAM role per Terragrunt stack (`bootstrap`, `persistent`, `disposable`, `ci/persistent`), least-privilege per stack — MUST NOT use one broad role with access across all state paths. Atlantis MUST NOT hold a role for `ci/disposable`. Spec 019's own workflow applies and destroys that state on every full-lifecycle run, using spec 019's own OIDC role (§17a of architecture.md) — never through Atlantis's plan/apply-on-merge flow.
