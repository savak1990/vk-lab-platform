...
- Rotation runbook (in the spec and README): first, generate a new pair with `ROTATE=1` into `civo-ca-cert-next.pem`. Then CIVO-082 adds a second trust anchor. Then CIVO-085 switches the issuer. Remove the old anchor after all certs (24 h) expired. Then promote the `-next` pair to the primary names (`git mv`) and commit that rename — only then, with the old-named files gone, is there anything to check for leftovers.
...
`scripts/civo-ca-init.sh` (new); `secrets/README.md`; `.gitignore` (`*.key`, `*-key.pem`, plus `!secrets/*/civo-ca-cert*.pem` to allow the public cert without also whitelisting rotation-candidate key filenames); the `Makefile` target `civo-ca-init`; `scripts/generate-secrets.sh` (the throwaway-project wiring, §2's addition).
...
- 2026-09-09 — final-review fix round: narrowed the `.gitignore` cert exception to `!secrets/*/civo-ca-cert*.pem` (the broad `!secrets/*/*.pem` left `civo-ca-key-next.pem` incorrectly trackable); rewrote the rotation runbook (spec §4 and `secrets/README.md`) to promote the `-next` pair to the primary filenames before any deletion, rather than deleting the live CA; added `INT`/`TERM` to `civo-ca-init.sh`'s cleanup trap so a Ctrl-C during the KMS call cannot leave a plaintext key on disk; sourced `scripts/lib/provider.sh` in `civo-ca-init.sh` for the correct default `PROJECT_NAME` under `PROVIDER=aws`; cleared `ROTATE` at `generate-secrets.sh`'s `civo-ca-init.sh` call site; changed the rotate-mode check to `[ "$ROTATE" = "1" ]`; added a refusal when `ROTATE=1` is run twice against an existing rotation candidate; corrected this section's prior `.gitignore`-verification and Task 4 Minor-findings claims; and added the `persistent-down.sh` caller to §12's Ruling. All fixes verified against throwaway project names; `secrets/vk-civo-lab/civo-ca-cert.pem` and `civo-ca-key.enc` untouched.
- 2026-09-20 — HETZ-018: `scripts/civo-ca-init.sh` became `scripts/ca-init.sh` and `make civo-ca-init` became `PROVIDER=<provider> make ca-init`. The file and key names are `${PROVIDER}-ca-cert[-next].pem` / `${PROVIDER}-ca-key[-next]` and the CN is `<project>-${PROVIDER}-workload-ca`; for civo all three expand to the existing names, so the committed CA is neither renamed nor re-encrypted. The script now refuses `PROVIDER=aws`. The §12 generate-if-missing trap is now reachable on hetzner too, and the operating order is recorded in `secrets/README.md`.
