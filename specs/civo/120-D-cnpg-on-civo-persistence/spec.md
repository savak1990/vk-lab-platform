...
- **No Civo e2e assertion exists.** `tests/e2e/postgres_test.go` asserts nothing about `ContinuousArchiving` or about a `completed` Backup, so a silent archiving failure would not fail a test. The read-only e2e ClusterRole would also need `backups.postgresql.cnpg.io` and `objectstores.barmancloud.cnpg.io`, which touches the AWS golden. Recorded here as a known gap; no spec owns it.
