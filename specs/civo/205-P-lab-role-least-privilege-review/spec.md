...
in §1. Also in scope: the **barman-cloud plugin's generated Role**
(carried from CIVO-180 §12). `Role/lab-postgres-barman-cloud` in
`cnpg-system` carries a secrets rule with an **empty** `resourceNames`:
`[""] ["secrets"] ["get","watch","list"]`. That is read access to every
Secret in the namespace, including `lab-postgres-app` and the Roles
Anywhere certificate `pgbackup-ra-cert`. It is generated by the plugin,
not written by this repository, and it applies cleanly here only because
no restrictive operator RBAC is in place (upstream plugin-barman-cloud
issue #892 reports it failing where such RBAC exists). Decide whether to
constrain it, accept it with a recorded rationale, or raise it upstream.
