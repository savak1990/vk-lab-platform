...
| PostgreSQL | `postgres/application.yaml`, `cluster.yaml`, `recovered-snapshot.yaml`, `priorityclass.yaml` | `ebs-delete`, nodeSelector, EBS snapshot handle | shared/refactor | operator hoisted; Cluster values-driven; `initdb` or `bootstrap.recovery` from the previous `serverName`; data restored by the barman-cloud plugin | values + ObjectStore + ScheduledBackup | 050, 120, 180, 185 |
...
| Postgres recovery | `argo-up.sh:167-197`, `argo-down.sh:54-113` | EBS snapshots | shared/refactor | **Delivered:** CNPG barman-cloud plugin, continuous WAL archiving to S3, generation-scoped `serverName`; best-effort `Backup` at teardown, `bootstrap.recovery` at bring-up (ADR 0032). Identical on both providers only after CIVO-185, which needs a redesign | branch, then removal | 045, 120, 180, 185 |
...
- **GitOps**: `gitops/templates/platform/shared/` *(proposed)* for hoisted components; `gitops/templates/platform/civo/` *(proposed)* for provider files; new values keys (`storage.className`, `capacity.spotAvoidance`, `postgres.nodeSelector`, `envoyGateway.reservedIp`, `envoyGateway.firewallId`, `awsIdentity.mode`, `externalDns.txtOwnerId`, `observability.kubeletInsecureTls`,
`observability.metricsServer.enabled`).
