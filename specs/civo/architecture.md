...
| Postgres recovery | `argo-up.sh:296-328`, `argo-down.sh:86-147` | EBS snapshots | shared/refactor | one-off dump Job at teardown; restore Job fired from `argo-up`; script presigns, Job speaks HTTP; manifests identical on both providers after CIVO-185 | branch, then removal | 045, 120, 180, 185 |
