...
- Credential: both the marketplace app and the upstream `civo` cloudprovider need a Civo API key as a Secret in `kube-system`. Use a **dedicated second API key** (Civo accounts may hold several). `argo-up` delivers it (never the main token). Then a Secret read in `kube-system` does not yield full account control. ADR 0030 must state this. **This spec owns the decision on where that second key is stored.** Spec 034 replaces the committed `secrets/*.enc` files with GitHub secrets and deliberately left this token out of its scope: the token is consumed by a controller inside the cluster, not by Terraform or a script on the operator's machine, so spec 034's environment-variable mechanism may not fit. Choose here, and record the choice against spec 034.
