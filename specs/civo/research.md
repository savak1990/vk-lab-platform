...
| Default cluster apps | Traefik (`traefik2-nodeport`) and metrics-server installed by default; removable via CLI `--remove-applications` or the `applications` field. Exact names to verify in the spike | https://www.civo.com/docs/kubernetes/create-a-cluster, civo/cli README | medium |
...
| Reserved IP | Assignable to a Kubernetes LB by annotation; stays in the account when released; chargeable; price not on the fetched page | https://www.civo.com/docs/networking/reserved-ip | medium (price unknown) |
...
| VolumeSnapshot | Not documented for `csi.civo.com`; spec 027 noted the driver's test suite hints at snapshot capability | none | low — spike |
| Cluster autoscaler | Marketplace app (`civo-cluster-autoscaler`, upstream cluster-autoscaler v1.25.0 listed), runs in `kube-system`, config `--nodes=min:max:poolname`, default `1:10:workers`; min 1 by default; account quota wins; scale-down order documented; no coordination with Terraform `node_count` | https://civo.com/docs/kubernetes/scaling-nodes, https://www.civo.com/marketplace/civo-cluster-autoscaler | high |
...
| Object store | Sized in 500 GB increments, hourly billing; per-GB price not on fetched pages | https://www.civo.com/docs/object-stores | low |
...
| CNPG barman plugin | `ObjectStore` with `s3Credentials` secret refs; `inheritFromIAMRole` behavior not confirmed on the fetched page | https://cloudnative-pg.io/plugin-barman-cloud/docs/usage/ | medium |
...
1. CIVO-020: create one Medium k3s cluster with a 5 GB PVC; snapshot; delete cluster; check account for snapshot and volume; create a second cluster in the same network; try static PV rebind by volume ID and `VolumeSnapshotContent` import. Expected cost under 2 USD.
