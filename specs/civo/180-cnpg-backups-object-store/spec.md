...
- Teardown: `argo-down` forces a final WAL switch and creates a `Backup` with `method: plugin`, then waits up to 600s. A failure or a timeout warns loudly and the teardown proceeds — continuous WAL archiving has already made every committed row durable.
