...
- [x] Civo render validated offline (structural check in `gitops-render-check.sh` + `kubeconform -strict`; the live-cluster `PROVIDER=civo make argo-up` sync is CIVO-045's acceptance criterion, not this spec's - moved there 2026-09-07, since 050 has no cluster to sync against on its own)
...
- 2026-09-07 — `/code-review high` (advisor-backed) found a real bug in the hoist: the observability RoleBinding, the BackendTrafficPolicy, and the grafana HTTPRoute rendered unconditionally after the hoist, but the `observability` namespace they live in/target is still aws-only (CIVO-160) - a real `target=civo` sync would fail apply with `namespaces "observability" not found`. Fixed in `02893ad`: `policies.yaml` moved back to `aws/` entirely; the grafana HTTPRoute doc and the observability RoleBinding doc went back under an aws-only gate in their still-hoisted files; `gitops-render-check.sh`'s object-set check now enforces the forbidden case (previously declared but not checked). Also fixed: a stale filename in `_helpers.tpl`'s own doc comment, several comments exceeding this repo's 3-line/no-doc-reference rule, and a misleading claim that `postgres.nodeSelector: {}` would clear the aws default (Helm deep-merges - it wouldn't). Deliberately not fixed: the 5x-duplicated spot-anti-affinity YAML stanza across `kube-prometheus-stack.yaml`/`loki.yaml`/`metrics-server.yaml` could factor into a `_helpers.tpl` partial like `platform.storageClassName` did, but risked an indentation bug for a non-correctness cleanup - left as a known simplification opportunity, not a defect.
- 2026-09-07 — merged to `main` (`14b9b2f`, alongside CIVO-040) and pushed to `origin/main`. Closed as DONE: the deferred live-cluster validation is CIVO-045's acceptance criterion now (cross-referenced there), not this spec's - 050 has no cluster of its own to sync against, only the render/structural/kubeconform checks, all green.
- 2026-09-20 — HETZ-016: `platform/civo/{cert-manager,identity,tls,postgres}` moved to `platform/shared/`, gated by the new `platform.selfManaged` helper. The civo render is unchanged object for object.
