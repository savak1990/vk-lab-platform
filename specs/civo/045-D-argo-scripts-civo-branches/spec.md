...
- 2026-09-20 — HETZ-016: the `= civo` gates for the TLS Secret round-trip, the CA Secret, the TERMINATING_KINDS probe and the PVC/PV waits became `!= aws`; `civo_import_tls_secret`/`civo_export_tls_secret` became `import_tls_secret`/`export_tls_secret`; the SSM path is now keyed by `${PROVIDER}` and expands unchanged for civo. Civo behaviour is unchanged.
