...
1. **Superseded by ADR 0012.** Originally: "For the `aws` target, Terraform MUST install Argo CD and the root Application... For the `local` target, a plain script/Makefile target installs Argo CD instead." ADR 0012 unifies both targets onto `scripts/argo-up.sh`/`scripts/argo-down.sh` (`make argo-up`/`make argo-down`) — Terraform is not involved in installing Argo CD for either target. The boundary this requirement protected is unchanged: nothing but Argo CD's own bootstrap and the root Application are installed outside Argo's own reconciliation, and no Kubernetes resource is Terraform-managed beyond that — this is still the exact boundary constitution §2 and architecture.md §7 draw, and the exact mistake ADR 0001 flags from `bg-tf-app`. See spec 006-1 for the teardown-ordering problem that drove this change.
2. The root Application MUST point at the `gitops/` directory in this repo and use Argo's own reconciliation (sync waves, app-of-apps) for everything beneath it — no imperative post-install scripting of Kubernetes resources from Terraform or CI. The root Application manifest MUST be a single manifest parameterized by a `target` value (`aws` | `local`), not two separate manifests (spec 022 Requirement 2); its umbrella chart MUST use that value to omit AWS-only child Applications when `target=local` (spec 022 Requirements 3, 14).
3. Once the root Application exists, any further change to what's installed on the cluster MUST go through Git + Argo sync (`aws`) or a local-working-directory sync (`local`, spec 022 Requirement 15), not through a second Terraform apply touching Kubernetes objects.
...
- After `terraform apply`, Argo CD is reachable (port-forward or later via the real NLB/Envoy path from spec 011) and shows the root Application as `Synced`/`Healthy` even with an empty or near-empty `gitops/` tree.
