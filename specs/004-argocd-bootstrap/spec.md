...
For the **`local` target** (minikube/kind), Argo CD is installed by a plain script/Makefile target instead of Terraform, and the same root Application manifest is applied with a different `target` value — see spec 020, which this spec's implementation MUST accommodate from the outset (a single, `target`-parameterized root Application manifest, not two divergent ones).
...
- After `terraform apply`, Argo CD is reachable (port-forward or later via the real ALB/Envoy path from spec 011) and shows the root Application as `Synced`/`Healthy` even with an empty or near-empty `gitops/` tree.
