...
**Depends on:** 003 through 008 (there must be something to observe: EKS, Karpenter, Postgres, Kafka all exist by now). Debezium (spec 024) lands after this spec — its dashboards are added retroactively, not a dependency of this spec's initial implementation.
...
- Kafka consumer-lag dashboards deserve particular attention now; Debezium/replication-slot-lag dashboards follow the same pattern when spec 024 adds them, given that spec's warning about WAL bloat from stalled slots.

## Testing / acceptance criteria

- Grafana shows live dashboards for Kubernetes node/pod health, Karpenter scaling activity, Kafka broker/topic metrics, and Postgres connection/replication metrics. Debezium connector health is added to this list when spec 024 lands.
