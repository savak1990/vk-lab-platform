...
- Reserve a slot for a required status check, to be pointed at spec 017's fast-validation workflow once it exists.
...
3. This spec MUST be completed before spec 016 (Atlantis) starts consuming PR events — Atlantis's plan-on-PR/apply-on-merge model assumes merges to `main` only happen through reviewed PRs.
4. Once spec 017's fast-validation workflow exists, its status check MUST be added as a required check on `main` — this spec does not block on 017 existing yet, but the required-check setting MUST be revisited and updated when 017 lands, not left permanently empty.
...
- If later specs want this Terraform-managed instead (e.g., via the `integrations/github` Terraform provider once spec 016's automation exists to apply it), that is a reasonable future improvement — record it as a follow-up, not a requirement of this spec.
- Revisit the required-status-check list twice more: once when spec 017 (fast validation) lands, and again if spec 018 (full lifecycle validation) or spec 016 (Atlantis) should also gate merges.
...
- A pull request against `main` can be merged only after any currently-required status checks pass (initially none, until spec 017 lands).
