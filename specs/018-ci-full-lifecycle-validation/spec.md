...
**Recommended model:** Opus for the run orchestration and postcondition checklist (the same reasoning burden as spec 013, now run unattended); Sonnet is fine for the scheduled cleanup job.
...
Implements a manually-triggered `platform-integration.yml` workflow that runs the full create→verify→write→destroy→verify-persistence→recreate→verify-recovery→destroy→verify-no-leaks sequence from spec 013, against CI's own isolated state — plus a scheduled safety-net cleanup job (ADR 0007):

- `.github/workflows/platform-integration.yml` — `workflow_dispatch`-triggered (or triggered on a trusted-context push to `main` touching `terraform/**`/`gitops/**`), running spec 013's exact sequence against `terraform/live/ci/{persistent,disposable}` state, verified using spec 021's Go/Ginkgo E2E suite. A `mode` input selects between two runs of the same workflow:
...
Excludes: the fast, lint-only PR checks (017); the kind-based GitOps integration test (022 — cheaper, AWS-free, runs on every relevant PR rather than being maintainer-gated); Atlantis's PR-triggered plan/apply on ordinary Terraform changes (016 — this spec runs the heavier, full-environment test, not routine plan/apply, and Atlantis holds no role for this spec's `ci/disposable` state); anything touching the personal lab's state, zone, or certificate. Also excludes the `local` target entirely — this spec's full lifecycle sequence is `aws`-target-only (spec 013 is what it runs, and spec 013 is `aws`-only).

## Requirements

1. This workflow MUST run the exact sequence from spec 013 (constitution §11, §12), against CI's own state, verified using spec 021's Go/Ginkgo E2E suite — not a reimplementation of that sequence or a parallel bash-based verification script.
2. CI infrastructure MUST be isolated from the personal lab environment (constitution §11). It MUST use separate state (`terraform/live/ci/persistent/`, `terraform/live/ci/disposable/`) and its own IAM role, distinct from spec 014's personal-lab role and spec 016's Atlantis roles. All three roles trust the one OIDC provider spec 001 creates (constitution §5) — this workflow can never touch personal data or resources.
...
- Create only this spec's own role, trusting the single OIDC provider spec 001 already created (do not create a second provider) — scope its trust policy and IAM permissions to `terraform/live/ci/*` state paths only, mirroring spec 014's personal-lab role pattern at the role level.
...
- The scheduled cleanup job can reuse the same postcondition-checking logic built in spec 013, scoped to the CI account/tag namespace, run on a cron trigger independent of any specific workflow run.

## Testing / acceptance criteria

- A manually-triggered `mode=routine` run completes the entire CREATE→...→VERIFY NO LEAKS sequence against `terraform/live/ci/` state successfully, using spec 021's E2E suite for verification, including HTTPS verification against CI's own delegated subdomain.
...
- CI's role cannot assume or touch anything scoped to the personal lab's state paths or DNS zone (spec 014), or Atlantis's per-stack roles (spec 016), and vice versa — and confirm it shares spec 001's single OIDC provider rather than a second one existing in the account.
