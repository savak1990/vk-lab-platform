...
**Estimated cost:** ~1.5–2 days · AWS runtime cost: none — this spec is pure Go tooling, reused by (not run standalone in) specs 018 and 022.
...
Introduces the platform's E2E verification framework, per ADR 0007: a Go/Ginkgo v2/Gomega test suite under `tests/e2e/`, with an `Environment` abstraction so the same service-level assertions run unmodified against a kind cluster (spec 022) or real EKS (spec 018).
...
11. Ginkgo labels MUST allow running a single service's tests in isolation (e.g. `ginkgo --label-filter=postgres`), so `make test-postgres`-style targets (spec 022) can invoke exactly one service's suite without running the rest.
...
- Structure the suite so `go vet`/`golangci-lint` (or whatever the repo eventually settles on for Go) can run as part of spec 017's fast validation once this code exists.
