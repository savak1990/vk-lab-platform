...
- `make bootstrap-down` is exercised at most once, deliberately, in a disposable/throwaway test AWS account (never against the real personal-lab account) to confirm: it refuses to run while Persistent or Disposable state exists, it requires its confirmation step, and it successfully removes the KMS key without touching the State layer's bucket. (Spec 015's OIDC provider is deliberately out of this command's reach — it lives in `terraform/live/account/` with its own `make account-down`, per ADR 0021. Every role trusting it — spec 016's, spec 020's — and Atlantis's compute from spec 018 are separate Bootstrap-lifecycle resources this command must still account for once those specs exist.)
