...
>    respective specs: spec 014 creates the personal-lab "normal deploy"
>    role, spec 018 creates the CI "privileged full-environment-test" role.
...
- One KMS key used later to encrypt each individual bootstrap-config file under `secrets/` (one file per secret/config value — never a combined blob, per constitution §5/§14). These files carry both runtime secrets (populated fully in spec 012) and non-secret private configuration needed earlier — notably `secrets/root-domain.enc`, consumed by spec 002 — so the decrypt-and-supply mechanism this spec establishes must be usable well before spec 012's full secrets lifecycle exists.
- One GitHub OIDC provider (`token.actions.githubusercontent.com`), a `terraform/live/bootstrap/github-oidc/` unit — the provider only, account-level and region-agnostic, created once and reused by every OIDC-authenticated consumer role this platform ever creates (spec 014's personal-lab role, spec 018's CI role). This unit creates no IAM role.
...
Excludes: the Terraform state bucket (moved to the State layer — see the scope amendments above), every OIDC-trusted IAM role (spec 014's personal-lab role, spec 018's CI role — this spec creates only the shared provider they trust), Atlantis's own per-stack IAM roles and compute (spec 016), networking (a dedicated VPC is deferred to spec 019 — EKS runs in the AWS default VPC until then, per spec 003), Route 53/ACM/Secrets Manager (002), EKS (003), any Kubernetes-facing IAM (Pod Identity roles are created alongside the workloads that need them, in later specs).
...
2. GitHub Actions → AWS authentication MUST use OIDC and temporary credentials; no long-lived AWS access keys may be stored as GitHub secrets (constitution §5). This spec creates the one account-level OIDC provider every such role trusts; it does not create any role itself (spec 014, spec 018) — but nothing built here may require a long-lived AWS key as a substitute in the meantime.
