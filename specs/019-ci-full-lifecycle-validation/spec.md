...
**Depends on:** 001-bootstrap (state/tagging conventions, and — since ADR 0007 — the one account-level GitHub OIDC provider this spec's CI role trusts), 002-persistent-foundation (delegated-subdomain pattern to replicate, one level down, for CI's own zone), 014-lifecycle (the exact `make up`/`make down` sequence this spec runs), 015-github-actions-lifecycle (this spec's CI-scoped role follows the same per-consumer role-scoping pattern as 015's personal-lab role — both trust spec 001's provider, neither creates its own), 017-atlantis-terraform-automation (this spec's CI-scoped IAM role follows the same per-stack scoping convention), 022-e2e-test-framework (the Go/Ginkgo suite this spec runs instead of ad hoc verification scripts)
