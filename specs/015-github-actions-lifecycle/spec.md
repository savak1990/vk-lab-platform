...
**Depends on:** 001-bootstrap (state backend/tagging conventions this spec's own Terraform, if any, follows), 014-lifecycle (the `make up`/`make down` targets this spec calls)

> **Scope amendment:** the GitHub OIDC provider and the OIDC-trusted IAM
> role these workflows assume were originally planned as part of spec 001.
> They move here instead: this is the first spec that actually runs a
> GitHub Actions workflow authenticating to AWS, so this spec now creates
> both the OIDC provider and its own personal-lab-scoped IAM role, in
...
- A GitHub OIDC provider (`token.actions.githubusercontent.com`) in AWS and a personal-lab-scoped IAM role trusting it — the first real consumer of GitHub OIDC federation in this platform (spec 017's Atlantis uses its own instance/task role instead, never OIDC).
...
1. `lab-up.yml`/`lab-down.yml` MUST be thin wrappers: check out the repo, assume the OIDC role created by this spec, and call the exact same `make up`/`make down` targets a developer would run locally — no divergent logic between local and GitHub execution (architecture.md §34, constitution's platform invariant #13).
...
- Create the OIDC provider and IAM role as their own Terraform (e.g. `terraform/live/bootstrap/github-oidc/`, a Bootstrap-lifecycle unit alongside spec 001's `kms` unit, inheriting the same `root.hcl` tagging/backend conventions — the state bucket itself lives one layer below, in `terraform/live/state/`, not inside Bootstrap) — a trust policy scoped to `repo:savak1990/vk-lab-platform:*` with a `sub` condition, not a blanket `repo:*:*` trust, and permissions limited to the personal-lab persistent/disposable state paths.
