#!/usr/bin/env bash
# Creates Persistent-lifecycle resources for the Hetzner target: the AWS
# secrets and backup units (the VPC is AWS-only, so it is excluded), then
# the Hetzner network, subnet and SSH key the disposable servers attach to.
# The aws target keeps its own inline Makefile recipe, unchanged.
set -euo pipefail
