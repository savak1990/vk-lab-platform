...
keys=$(aws s3api list-objects-v2 --bucket "$BUCKET" --prefix "account/" --region "$LAB_ACCOUNT_REGION" \
  --query "Contents[?ends_with(Key, 'terraform.tfstate')].Key" --output text)

total=0
if [ -n "$keys" ] && [ "$keys" != "None" ]; then
  for key in $keys; do
    aws s3api get-object --bucket "$BUCKET" --key "$key" --region "$LAB_ACCOUNT_REGION" "$TMP_DIR/state.json" >/dev/null
...
aws s3api list-object-versions --bucket "$BUCKET" --region "$LAB_ACCOUNT_REGION" \
...
for f in versions markers; do
  ...
  while [ "$offset" -lt "$total" ]; do
    ...
    if jq -e '.Objects | length > 0' "$TMP_DIR/${f}_batch.json" >/dev/null; then
      aws s3api delete-objects --bucket "$BUCKET" --region "$LAB_ACCOUNT_REGION" --delete "file://$TMP_DIR/${f}_batch.json"
    fi
    offset=$((offset + 1000))
  done
done

aws s3api delete-bucket --bucket "$BUCKET" --region "$LAB_ACCOUNT_REGION"
