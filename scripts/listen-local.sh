...
if [ "$PROVIDER" != "local" ]; then
  echo "listen-local.sh: PROVIDER=$PROVIDER reaches the gateway over DNS - there is nothing to forward." >&2
  exit 1
fi

LOCAL_PORT="${LOCAL_PORT:-8080}"
pidfile="$REPO_ROOT/.kube/$PROJECT_NAME-listen.pid"
logfile="$REPO_ROOT/.kube/$PROJECT_NAME-listen.log"

if [ -f "$pidfile" ] && kill -0 "$(cat "$pidfile")" 2>/dev/null; then
  echo "LISTEN: already forwarding on pid $(cat "$pidfile") - 'make unlisten' stops it."
  exit 0
fi
rm -f "$pidfile"
...
if command -v lsof >/dev/null 2>&1; then
  ...
  if [ -n "$holder" ]; then
    echo "listen-local.sh: port $LOCAL_PORT is already held by '$holder'." >&2
    echo "listen-local.sh: stop it, or choose another port with LOCAL_PORT=<n>." >&2
    exit 1
  fi
fi
...
if [ -z "$service" ]; then
  echo "listen-local.sh: no gateway Service in namespace envoy - has 'make up' finished?" >&2
  exit 1
fi
...
for _ in $(seq 1 30); do
  if grep -q "Forwarding from 127.0.0.1:$LOCAL_PORT" "$logfile" 2>/dev/null; then
    echo "LISTEN: Argo CD is http://localhost:$LOCAL_PORT - admin / '${LOCAL_ARGOCD_PASSWORD:-test}'."
    echo "LISTEN: Grafana is http://localhost:$LOCAL_PORT/grafana - admin / '${LOCAL_GRAFANA_PASSWORD:-test}'."
    echo "LISTEN: 'make unlisten' stops it. Log: $logfile"
    exit 0
  fi
...
echo "listen-local.sh: the forward did not come up on 127.0.0.1:$LOCAL_PORT." >&2
echo "listen-local.sh: LOCAL_PORT=<n> picks another port. Last lines:" >&2
tail -5 "$logfile" >&2
exit 1
