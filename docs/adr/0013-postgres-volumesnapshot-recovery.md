...
> **Note (2026-09-17):** Superseded by
> [ADR 0033](0033-barman-cloud-plugin-backups-on-aws.md). Both targets now
> back up PostgreSQL through the CNPG barman-cloud plugin, and the
> `VolumeSnapshot` mechanism below is removed. Civo never used it — Civo's
> CSI driver has no snapshot capability.
...
**Retention:** newest 2 snapshots, enforced by the scripts calling the AWS API directly — not CNPG's `.spec.backup.retentionPolicy`, which has an open, unresolved pruning bug (cloudnative-pg/cloudnative-pg#8599) as of this writing. `persistent-down.sh` deletes *all* snapshots for this tag, alongside its existing retained-volume cleanup (ADR 0008) — a full wipe is expected when tearing down the persistent tier. `bootstrap-down.sh`'s own state-emptiness check can't see these (they live outside Terraform state) — it relies on `persistent-down.sh` (which `full-down` always runs first) having already deleted them, rather than duplicating that check itself.
