...
Spec 017 introduces Atlantis to run `terraform plan` on pull requests and `terraform apply` on merge, so Terraform changes go through a reviewed PR instead of a local `terraform apply`. Atlantis needs somewhere to run.
...
Atlantis assumes a distinct IAM role per Terragrunt stack (`bootstrap`, `persistent`, `disposable`, `ci/persistent`, `ci/disposable`), matching the least-privilege, per-stack role pattern spec 001 already establishes for other actors.
