...
| 2026-09-22 | Hetzner default location changed to `fsn1`, network zone unchanged | The 2026-09-11 row chose `nbg1` on the stated grounds that nothing material separates the three, which leaves the default free to follow practice: every real Hetzner cycle has run in `fsn1`, and HETZ-175's create-call probes proved it sells five node types after the catalogue had recorded it as empty. The state bucket name carries the region, so an existing project in another location is refused by `state-up`'s guard with the teardown command to run, rather than silently given a second bucket | staying on `nbg1` and passing `REGION=fsn1` by hand every run; `hel1`, which sells four types rather than five |
...
| 2026-09-20 | Hetzner bootstrap driver decided: the control plane's cloud-init runs `kubeadm init` and the Cilium install at first boot; the script joins the workers, fetches the kubeconfig and waits for Ready | No secret ever enters `user_data` — kubeadm generates the CA and the join token on the control plane and they stay there; no relay service; Terraform still returns at "server is running", and an init failure is read with `cloud-init status --long` and `journalctl -u kubelet`, which the bootstrap script prints on timeout | a script driving init, Cilium and join over SSH (five round trips, init output in the operator's terminal); a Terraform-generated CA and token in `user_data` (private key in state and in instance metadata); the control plane serving the join command on the private network (a moving part that fails silently) |
...
- Earlier research: the withdrawn alternative-cloud-targets spec (superseded by this package)
